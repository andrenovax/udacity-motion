...
double close_to_main_goal_cost_spiral(const std::vector<PathPoint>& spiral,
  ...
  auto &last_spiral_point = spiral.back();
  auto delta_x = last_spiral_point.x - main_goal.location.x;  // <- Update
  auto delta_y = last_spiral_point.y - main_goal.location.y;  // <- Update
  auto delta_z = last_spiral_point.z - main_goal.location.z;  // <- Update
